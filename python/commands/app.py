...
def resource_group_strategy(resource_group: str, location: str) -> Tuple[az.ResourceGroup, bool]:
    ...
    _log.debug(f"resource_group_data: {0}".format(resource_group_data))
...
def key_vault_strategy(location: str, resource_group: str, key_vault: str):
    ...
    _log.debug(f"key_vault_data: {0}".format(key_vault_data))
...
def ad_group_strategy(ad_member_id: str, ad_group: str = "main-ad-group") -> Tuple[az.AdGroup, bool]:
    ...
    scope: str = f"/subscriptions/{0}".format(_az.subscription_id)
...
def login_service_principal_strategy(location: str, resource_group: str, key_vault: str, service_principal: str, auth_dir: str) -> Tuple[az.ServicePrincipal, bool]:
    ...
    if sh.path_exists(service_principal_path, "f"):
        ...
        _log.debug(f"service_principal_data: {0}".format(
...
def login_strategy(tenant, subscription, location, resource_group, key_vault, sp_name, auth_dir, retry=True):
    ...
    if _az.is_signed_in and not service_principal:
        ...
        _az = login_strategy(tenant, subscription, location,
...
def login_devops_strategy(user: str, location: str, resource_group: str, key_vault: str, auth_dir: str, retry: Optional[bool] = True) -> bool:
    ...
    if not user_is_signed_in:
        if _az.devops_pat:
            ...
            _log.error(f"this PAT must be in '{0}' as '{1}' before repeating your previous command".format(
...
def application_strategy(tenant: str, root_dir: str, solution: str, project: str, strat: str, environment: str,
    ...
    app_dir = sh.path_join(
        root_dir, solution) if use_solution_dir else sh.path_join(root_dir, project)
    ...
    if strat == "identity":
        ...
        raw_app_name = f"{0}-{1}".format(project, environment)
        app_name = sh.format_resource(raw_app_name)
        _log.info(f"app registration name: {0}".format(app_name))
    ...
    _log.debug(f"Project Name: {0}".format(project))

    # Create project directory
    project_dir: str = sh.path_join(
        app_dir, project) if use_solution_dir else app_dir
    _log.debug(f"checking for project dir ({0})...".format(project_dir))
    ...
    if not project_dir_exists:
        ...
        _log.info(
            f"successfully created project directory: {0}".format(project_dir))

    # Create ASP.NET Core project
    project_file: str = sh.path_join(project_dir, "{0}.csproj".format(project))
    _log.debug(f"checking for project ({0})...".format(project_file))
    ...
    if not project_exists:
        ...
        _log.info(f"successfully created project: {0}".format(
            project_succeeded))
    ...
    solution_file: str = sh.path_join(app_dir, "{0}.sln".format(
        solution)) if use_solution_dir else sh.path_join(app_dir, "{0}.sln".format(project))
    _log.debug(f"checking for solution ({0})...".format(solution_file))
    ...
    if not solution_exists:
        ...
        _log.info(f"successfully created solution: {0}".format(sln_succeeded))
    ...
    if not project_added:
        _log.error(f"failed to add project: {0}".format(project))
        sh.process_fail()
    ...
    _log.debug(f"NuGet packages_to_install: {0}".format(packages_to_install))
    for package in packages_to_install:
        package_succeeded: bool = net.project_package_add(project_dir, package)
        if not package_succeeded:
            _log.error(f"failed to add package: {0}".format(package))
            sh.process_fail()
...
def repository_strategy(organization: str, root_dir: str, app_name: str, source="", gitignore_path="", remote_alias="origin") -> bool:
    ...
    if source == "github":
        remote_path = f"https://github.com/{0}/{1}".format(
            organization, app_name)
        _log.debug(
            f"source repository (GitHub) remote: {0}".format(remote_path))
    elif source == "tfsgit":
        remote_path = f"https://dev.azure.com/{0}/{1}".format(
            organization, app_name)
        _log.debug(f"source repository (Azure) remote: {0}".format(remote_path))
        return False
    else:
    ...
    _log.debug(f"checking for repository directory ({0})...".format(app_dir))
    app_dir_exists: bool = sh.path_exists(app_dir, "d")
    if not app_dir_exists:
        _log.warning("could not locate repository directory, creating...")
        sh.directory_create(app_dir)
        _log.info(
            f"successfully created repository directory: {0}".format(app_dir))

    # Initialize Git repository directory
    repo_dir_exists: bool = git.repo_exists(app_dir, is_bare)
    if repo_dir_exists:
        _log.debug(f"Successfully found {0} repository".format(repo_descriptor))
    else:
        _log.debug(f"Unable to locate {0} repository".format(repo_descriptor))
        display_path: str = app_dir if (
            is_bare) else f"{0}/.git".format(app_dir)
        _log.info(
            f"Repository not found ({0}), initializing...".format(display_path))
        # Initialize the repository
        (repo_exists, repo_changed) = git.repo_create(app_dir, is_bare)
        if repo_exists:
            _log.info(f"successfully created {0} repository!".format(
                repo_descriptor))
        else:
            _log.error(
                f"Unable to create {0} repository".format(repo_descriptor))
            sh.process_fail()
...
def deployment_group_strategy(tenant: str, sp_name: str, project: str, environment: str, location: str, arm: str) -> Tuple[bool, bool]:
    ...
    rg_name: str = sh.format_resource(f"{0}-{1}".format(project, environment))
    _log.debug(f"rg_name: {0}".format(rg_name))
    ...
    parameters_json: Dict[str, Dict[str, Any]
                          ] = az.ArmParameters(parameters_file).content
    ...
    # Convert parameters JSON to CLI-ready parameters
    _log.debug(f"parameters_json: {0}".format(parameters_json))
    parameters: List[str] = _json_to_parameters(parameters_json)
    _log.debug(f"parameters: {0}".format(parameters))

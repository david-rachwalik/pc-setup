...
class GitController(object):
    def __init__(self):
        # logger.debug("(GitController): Init")
        ...
        if (args.action in ["status", "reset"]):
            logger.debug("(GitController): skip bare repository check for '{0}' action".format(args.action))
        else:
            no_create = (args.action != "push") # fail when not 'push' action and repo is missing
            self.bare_repository = Repository(args.remote_path, no_create=no_create, force=True)
            logger.debug("(GitController): bare_repository exists: {0}".format(self.bare_repository.exists))
        ...
        logger.debug("(GitController): work_repository exists: {0}".format(self.work_repository.exists))
        ...
        if not remote_result:
            logger.error("(GitController): Error occurred updating remote path")
            sh.process_fail()
        ...
        if args.action in ["push", "pull"] and len(args.gitignore_path) > 0:
            ...
            if sh.path_exists(file_dest, "f"):
                ...
                if not hash_result:
                    logger.debug("(GitController): '.gitignore' hashes don't match, updating...")
                    update_result = sh.file_copy(file_src, file_dest)
                    if update_result:
                        logger.debug("(GitController): '.gitignore' was successfully updated!")
                    else:
                        logger.debug("(GitController): '.gitignore' failed to be updated")
                        sh.process_fail()
                else:
                    logger.debug("(GitController): '.gitignore' is already up-to-date")
            else:
                logger.debug("(GitController): '.gitignore' is missing, adding...")
                add_result = sh.file_copy(file_src, file_dest)
                if add_result:
                    logger.debug("(GitController): '.gitignore' was successfully added!")
                else:
                    logger.debug("(GitController): '.gitignore' failed to be added")
                    sh.process_fail()
        ...
        logger.debug("(GitController): work-tree is clean: {0}".format(work_tree_is_clean))
        ...
        logger.debug("(GitController): master_local_branch_exists: {0}".format(master_local_branch_exists))
        logger.debug("(GitController): master_remote_branch_exists: {0}".format(master_remote_branch_exists))
        stash_local_branch_exists = bp_git.branch_exists(args.local_path, stash_branch, branches=local_branches)
        logger.debug("(GitController): stash_local_branch_exists: {0}".format(stash_local_branch_exists))
        ...
        logger.debug("(GitController): version_local_branch_exists: {0}".format(version_local_branch_exists))
        logger.debug("(GitController): version_remote_branch_exists: {0}".format(version_remote_branch_exists))

        # Fail early when attempting to pull/delete a dirty work-tree
        if args.action in ["pull", "delete"] and not work_tree_is_clean:
            logger.error("(GitController): Unable to perform '{0}' action with dirty work-tree".format(args.action))
            sh.process_fail()
        ...
        if args.action == "delete" and args.branch == master_branch:
            logger.error("(GitController): Unable to delete the '{0}' branch".format(master_branch))
            sh.process_fail()
        ...
        if args.action == "pull":
            if not master_remote_branch_exists:
                logger.error("(GitController): Unable to perform '{0}' action; '{1}/{2}' branch not found".format(args.action, args.remote_alias, master_branch))
                sh.process_fail()
            if not version_remote_branch_exists:
                logger.error("(GitController): Unable to perform '{0}' action; '{1}/{2}' branch not found".format(args.action, args.remote_alias, args.branch))
                sh.process_fail()
        ...
        if args.action in ["push", "pull"] and not master_local_branch_exists:
            if master_remote_branch_exists:
                ...
                logger.debug("(GitController): checked out '{0}/{1}' branch".format(args.remote_alias, master_branch))
                master_local_branch_exists = bp_git.branch_create(args.local_path, master_branch)
                logger.debug("(GitController): created local '{1}' branch from remote '{0}/{1}' branch".format(args.remote_alias, master_branch))
            else:
                # Push initial 'master' branch to remote when not already there
                logger.warning("(GitController): '{0}' branch is missing, creating initial commit...".format(master_branch))
                ...
                logger.debug("(GitController): initial push to '{0}/{1}' has succeeded: {2}".format(args.remote_alias, master_branch, push_succeeded))
        ...
        if args.action == "push" and version_remote_branch_exists and not work_tree_is_clean:
            logger.debug("(GitController): preparing to stash work-tree changes...")

            # Create 'my-stash' branch from local 'master' branch
            bp_git.branch_switch(args.local_path, master_branch)
            logger.debug("(GitController): checked out '{0}' branch".format(master_branch))

            # Check if stash branch already exists, delete if so
            if stash_local_branch_exists:
                logger.debug("(GitController): previous '{0}' branch was detected, removing...".format(work_tree_is_clean))
                bp_git.branch_delete(args.local_path, stash_branch)
                logger.debug("(GitController): '{0}' branch was deleted".format(work_tree_is_clean))
            ...
            logger.debug("(GitController): created and checked out '{0}' branch".format(stash_branch))
            (commit_succeeded, commit_changed) = bp_git.work_commit(args.local_path)
            logger.debug("(GitController): commit for '{0}' branch succeeded: {1}".format(stash_branch, commit_succeeded))
        ...
        if args.action in ["push", "pull"]:
            ...
            if not version_local_branch_exists:
                if version_remote_branch_exists:
                    ...
                    logger.debug("(GitController): checked out '{0}/{1}' branch".format(args.remote_alias, args.branch))
                    version_local_branch_exists = bp_git.branch_create(args.local_path, args.branch)
                    logger.debug("(GitController): created local '{1}' branch from remote '{0}/{1}' branch".format(args.remote_alias, args.branch))
                else:
                    # Create local 'version' branch from 'master' branch
                    bp_git.branch_switch(args.local_path, master_branch)
                    logger.debug("(GitController): checked out '{0}' branch".format(master_branch))
                    version_local_branch_exists = bp_git.branch_create(args.local_path, args.branch)
                    logger.debug("(GitController): created local '{1}' branch from '{0}' branch".format(master_branch, args.branch))
                version_branch_just_created = True
                
                # Verify successfully created 'version' branch
                if not version_local_branch_exists:
                    logger.error("(GitController): local '{0}' branch still appears to be missing; failed to create".format(args.branch))
                    sh.process_fail()
            ...
            if args.action == "push":
                ...
                if not work_tree_is_clean and not version_remote_branch_exists:
                    (commit_succeeded, commit_changed) = bp_git.work_commit(args.local_path)
                    logger.debug("(GitController): commit for '{0}' branch succeeded: {1}".format(args.branch, commit_succeeded))
                    # Verify work-tree status (should always be clean after commit)
                    work_tree_is_clean = bp_git.work_status(args.local_path)
                    logger.debug("(GitController): work_tree_is_clean: {0}".format(work_tree_is_clean))
            ...
            if args.action in ["push", "pull"]:
                if version_remote_branch_exists and not version_branch_just_created:
                    ...
                    logger.debug("(GitController): merge '{0}/{1}' branch into local '{0}' branch".format(args.remote_alias, args.branch))
                    (succeeded, changed) = bp_git.work_merge(args.local_path, args.branch, args.remote_alias)
                    logger.debug("(GitController): '{0}/{1}' branch merge succeeded: {2}".format(args.remote_alias, args.branch, succeeded))


            # Rebase local 'my-stash' branch onto local 'version' branch
            if args.action == "push" and stash_local_branch_exists:
                bp_git.branch_switch(args.local_path, stash_branch)
                logger.debug("(GitController): checked out '{0}' branch".format(stash_branch))
                logger.debug("(GitController): rebasing '{0}' branch onto local '{1}' branch...".format(stash_branch, args.branch))
                (succeeded, changed) = bp_git.work_rebase(args.local_path, args.branch)
                logger.debug("(GitController): '{0}' branch rebase succeeded: {1}".format(stash_branch, succeeded))
                # Checkout local 'version' branch
                bp_git.branch_switch(args.local_path, args.branch)
        ...
        if args.action == "reset":
            ...
            logger.info("(GitController): ::mock:: reset action")
            ...
            logger.debug("(GitController): deleting '{0}' branch...".format(args.branch))
            ...
            logger.debug("(GitController): '{0}' branch deleted".format(args.branch))
        ...
        if args.action == "push":
            ...
            logger.debug("(GitController): pushing '{0}' branch to {1}".format(args.branch, args.remote_alias))
            (push_succeeded, push_changed) = bp_git.work_push(args.local_path, args.branch, args.remote_alias)
            logger.debug("(GitController): '{0}/{1}' branch push has succeeded: {2}".format(args.remote_alias, args.branch, push_succeeded))
